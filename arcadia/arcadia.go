...
type Arcadia struct {
	...
	// seenTransactions *emap.EMap[*chain.Transaction]
}
...
// @todo implement send builder chan
func NewArcadiaClient(url string, currEpoch uint64, currEpochBuilderPubKey *bls.PublicKey, availNs [][]byte, vm VM) *Arcadia {
	...
	return &Arcadia{
		...
		// seenTransactions:       emap.NewEMap[*chain.Transaction](),
	}
}
			...
			for {
				...
				case chunk := <-cli.incomingChunks:
					...
					if err != nil {
						...
						// @todo handle error and chunk maps
...
func (cli *Arcadia) HandleRollupChunks(chunk *ArcadiaChunk) error {
	...
	// @todo all checks are not done yet.
...
// @todo allocate set of cores for issuing preconfs.
func (cli *Arcadia) IssuePreconfs() error {
