...
// @todo integrate build chunk with anchor.
func BuildChunk(ctx context.Context, vm VM) (*Chunk, error) {
	...
	for time.Since(start) < vm.GetTargetChunkBuildDuration() {
		...
		nextUnits, err := fees.Add(chunkUnits, txUnits)
...
func (c *Chunk) Digest() ([]byte, error) {
	size := consts.Int64Len + consts.IntLen + codec.CummSize(c.Txs) + ids.NodeIDLen + bls.PublicKeyLen
...
func (c *Chunk) Size() int {
	return consts.Int64Len + consts.IntLen + codec.CummSize(c.Txs) + ids.NodeIDLen + codec.AddressLen + bls.PublicKeyLen + bls.SignatureLen
}

func (c *Chunk) Units(sm StateManager, r Rules) (fees.Dimensions, error) {
	if c.units != nil {
		return *c.units, nil
	}
	units := fees.Dimensions{}
	for _, tx := range c.Txs {
		...
		nextUnits, err := fees.Add(units, txUnits)
		if err != nil {
			return fees.Dimensions{}, err
		}
		units = nextUnits
...
func (c *ChunkSignature) Size() int {
	return ids.IDLen + consts.Int64Len + bls.PublicKeyLen + bls.SignatureLen
}
...
func (c *ChunkSignature) Digest() ([]byte, error) {
	size := ids.IDLen + consts.Int64Len
	p := codec.NewWriter(size, consts.NetworkSizeLimit)
...
func (c *ChunkCertificate) Size() int {
	...
	return ids.IDLen + consts.Int64Len + codec.BytesLen(signers) + bls.SignatureLen
}
...
func (c *FilteredChunk) Size() int {
	return ids.IDLen + consts.Int64Len + ids.NodeIDLen + codec.AddressLen + consts.IntLen + codec.CummSize(c.Txs) + consts.Uint64Len
}
