...
// It is good to call, Insert after calling prepare to the batch.
func (b *Batch) Insert(_ context.Context, k []byte, value []byte) error {
	key := string(k)
	start, err := b.writePut(string(key), value)
	if err != nil {
		return err
	}
...
// It is good to call, Remove after calling prepare to the batch.
func (b *Batch) Remove(_ context.Context, k []byte) error {
	key := string(k)
	// Check input
	if len(key) > int(consts.MaxUint16) {
...
// GetValue returns the value associated with the key. Get value is not callable/blocked when prepare is called to the batch and write is not called.
...
// Write unlocks keylock and commitlock, reads to data base should be possible after write.
// Write returns any error and checksum of the batch.
